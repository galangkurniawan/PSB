...
///==============================================================registrasi
	...
	function daftar(){
		$cek = $this->db->query("SELECT * FROM pendaftaran where nama='".$this->input->post('nama')."'")->num_rows();
		$u = $_POST['nem'];
		if($cek<=0){
			...
			elseif (!preg_match($var, $input)) {
				echo "<script>
						alert('Inputan Nem Tidak Sesuai');
						javascript:history.back();
					</script>";
			}
			elseif (!preg_match($vari, $nilai)) {
				echo "<script>
						alert('Inputan Nama Tidak Sesuai');
						javascript:history.back();
					</script>";					
			}
			elseif (!preg_match($varia, $tmpt)) {
				echo "<script>
						alert('Inputan Tempat Lahir Tidak Sesuai');
						javascript:history.back();
					</script>";	
					}
			elseif (!preg_match($no, $hp)) {
				echo "<script>
						alert('Inputan Nomer Telepon Tidak Sesuai');
						javascript:history.back();
					</script>";	
					}
			elseif (!preg_match($b, $a)) {
				echo "<script>
						alert('Inputan Nama Asal Sekolah Tidak Sesuai');
						javascript:history.back();
					</script>";	
					}
			elseif ($u >= 40) {
				echo "<script>
						alert('Inputan Nem Tidak Sesuai (Entuk Biji semono soko ndi!!!)');
						javascript:history.back();
					</script>";
					}		
			else{
